...
object DropDown2
{
	...
	{
		val dd = new DropDown2[A, C](context.actorHandler, noResultsView, icon,
			new BackgroundDrawer(context.highlightColor, Normal), selectionPromptText, context.font, context.textColor,
			displayFunction, context.textAlignment, context.insets, context.insets.withLowPriority, borderColor,
...
(override protected val actorHandler: ActorHandler, override protected val noResultsView: AwtStackable, icon: Image,
 selectionDrawer: CustomDrawer, selectionPrompt: Prompt, defaultFont: Font,
	...
	extends DropDownFieldLike[A, C](selectionDrawer, betweenDisplaysMargin, displayStackLayout, contentPointer, valuePointer)
{
	...
	private val noValueContext = TextDrawContext(selectionPrompt.font, selectionPrompt.color, textAlignment)
	private val valueSelectedContext = TextDrawContext(defaultFont, defaultTextColor, textAlignment)
	
	private val textLabel = new TextLabel(selectionPrompt.text, selectionPrompt.font, selectionPrompt.color,
		textInsets.mapRight { _.noMax.withLowPriority }, textAlignment, textHasMinWidth)
